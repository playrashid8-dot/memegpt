...
export default function TickerTape() {
  ...
  return (
    <div
      className="relative w-full overflow-hidden border-y border-neon/15 bg-black/75 backdrop-blur-lg group shadow-[0_0_32px_rgba(0,255,136,0.04)]"
      aria-label="Live token ticker"
    >
      <div className="absolute inset-y-0 left-0 w-16 sm:w-28 bg-gradient-to-r from-black via-black/90 to-transparent z-10 pointer-events-none" />
      <div className="absolute inset-y-0 right-0 w-16 sm:w-28 bg-gradient-to-l from-black via-black/90 to-transparent z-10 pointer-events-none" />
      ...
      <div className="flex py-3 sm:py-3.5 group-hover:[animation-play-state:paused] animate-ticker whitespace-nowrap select-none">
        {items.map((item, i) => (
          <span
            key={`${item.token}-${i}`}
            className="inline-flex items-center gap-2 sm:gap-2.5 mx-4 sm:mx-7 text-[11px] sm:text-sm font-mono shrink-0"
          >
