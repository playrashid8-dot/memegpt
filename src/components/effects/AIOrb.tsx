...
export default function AIOrb({ lite = false }: AIOrbProps) {
  const { canAnimate } = useMotionProfile();
  const staticMode = lite || !canAnimate;
  const particles = staticMode ? [] : lite ? LITE_ORBIT : ORBIT_PARTICLES;
  const rings = staticMode ? [0] : lite ? [0, 2] : [0, 1, 2, 3];
  ...
  return (
    <div
      ...
      } ${staticMode ? "" : lite ? "" : "animate-hologram"}`}
      aria-hidden="true"
    >
      {!lite && !staticMode && (
      ...
      {staticMode ? (
        <div
          ...
          style={{
            background:
              "radial-gradient(circle at 35% 30%, rgba(0,255,136,0.4), rgba(0,200,100,0.16) 35%, rgba(0,80,50,0.28) 55%, rgba(0,0,0,0.88) 75%)",
            boxShadow: "inset 0 0 28px rgba(0,255,136,0.18), 0 0 32px rgba(0,255,136,0.1)",
          }}
        >
          <div className="absolute inset-0 flex items-center justify-center">
            <span className={`drop-shadow-[0_0_12px_rgba(0,255,136,0.45)] ${lite ? "text-2xl sm:text-4xl md:text-5xl" : "text-3xl sm:text-4xl md:text-5xl"}`}>
              🧠
            </span>
          </div>
        </div>
        ...
        <>
          <motion.div
            ...
            className={`absolute -inset-4 sm:-inset-6 rounded-full bg-neon/10 ${
          ...
          {!lite && (
            <motion.div
              ...
              className="absolute -inset-3 rounded-full bg-cyan/8 blur-[48px] hidden sm:block"
            />
          )}
          ...
          <motion.div
            ...
            className="absolute inset-3 rounded-full border border-dashed border-neon/22"
          />
          {!lite && (
            <motion.div
              animate={{ rotate: 360 }}
              transition={{ duration: 18, repeat: Infinity, ease: "linear" }}
              className="absolute inset-6 rounded-full border border-dotted border-neon/12 hidden sm:block"
            />
          )}
          ...
          {!lite && (
            <motion.div
              ...
              className="absolute inset-0 rounded-full border border-neon/20 hidden sm:block"
            />
          )}
        </>
      )}
