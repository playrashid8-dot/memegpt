        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-14 lg:gap-20 items-center">
          ...
          >
            {canAnimate && !isMobile ? (
              ...
              <>
                <div className="absolute inset-0 rounded-full border border-dashed border-neon/20" />
                <div className="absolute inset-4 rounded-full border border-neon/10" />
                <div className="absolute inset-6 rounded-full bg-neon/10 blur-2xl" />
              </>
            )}
                ...
                (acc, item, i) => {
                  ...
                  acc.elements.push(
                    <motion.circle
                      ...
                      style={{ filter: isMobile ? undefined : `drop-shadow(0 0 12px ${item.color}90)` }}
                    />
                  );
            ...
            <div className="absolute inset-0 flex flex-col items-center justify-center z-20">
              <span className={`font-display text-5xl md:text-6xl font-black ${isMobile ? "gradient-text" : "gradient-text-shimmer"}`}>
                1B
              </span>
          ...
          <div className="space-y-4 sm:space-y-5">
            {TOKENOMICS.map((item, i) => (
              <motion.div
                ...
                className={`${cardClass} rounded-xl p-4 sm:p-5 corner-accent holographic-border`}
              >
                <div className="flex items-center justify-between mb-3">
                  <div className="flex items-center gap-3">
                    <div
                      className="w-3.5 h-3.5 rounded-full"
                      style={{ backgroundColor: item.color, boxShadow: isMobile ? undefined : `0 0 15px ${item.color}` }}
                    />
                ...
                <div className="h-2.5 rounded-full bg-white/[0.04] overflow-hidden border border-white/[0.04]">
                  <motion.div
                    ...
                    style={{ backgroundColor: item.color, boxShadow: isMobile ? undefined : `0 0 12px ${item.color}70` }}
                  />
                </div>
              </motion.div>
            ))}
